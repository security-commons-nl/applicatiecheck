// Gedeeld build-script, gekopieerd uit ai-gebruik-in-beeld. Wijkt op twee punten af: de query-tab en de
// tabnavigatie verschijnen alleen als er iets in staat. Hoort op één plek te wonen (.github); tot die
// centralisatie: wijzig beide.
...
function buildPage() {
  ...
  const allTabs = QUERY_FILES.length ? [...TABS, { id: 'queries', label: "Query's" }] : TABS;
  ...
  const sections = [
    ...
    ...(QUERY_FILES.length ? [`<section id="queries" class="tab-panel" aria-label="Query's">
${buildQueriesSection()}
</section>`] : []),
  ].join('\n');
  return `<!doctype html>
...
${nav ? `  <nav class="tabs inner" aria-label="Hoofdstukken">
${nav}
  </nav>` : ''}
</header>
<main class="inner">
